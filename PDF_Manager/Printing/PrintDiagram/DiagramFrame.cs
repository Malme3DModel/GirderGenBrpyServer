...
namespace PDF_Manager.Printing
{
    class DiagramFrame
    {
        ...
        public void printInit(PdfDocument mc, PrintData data)
        {
            ...
            // Ver2 より古ければ 1
            this.isOlderVer2 = (data.isOlderVer2) ? 1 : -1;
            ...
            foreach (var n in this.Node.Nodes.Values)
            {
                ...
                if (n.x > RightBottom.X)
                    RightBottom.X = n.x;
                if (this.isOlderVer2 * n.y < RightBottom.Y)
                    RightBottom.Y = this.isOlderVer2 * n.y;
                if (this.isOlderVer2 * n.y > LeftTop.Y)
                    LeftTop.Y = this.isOlderVer2 * n.y;
            }
        ...
        public void printFrame(int currentArea = 1, bool isNode = true)
        {
            ...
            {
                var pi = this.Node.GetNodePos(mm.ni);
                var pj = this.Node.GetNodePos(mm.nj);

                //n スケール調整
                var xi = (pi.x - this._CenterPos.X) * this.scaleX;
                var yi = -((this.isOlderVer2 * pi.y) - this._CenterPos.Y) * this.scaleY;
                var xj = (pj.x - this._CenterPos.X) * this.scaleX;
                var yj = -((this.isOlderVer2 * pj.y) - this._CenterPos.Y) * this.scaleY;
            ...
            if(isNode == true)
            {
                ...
                foreach (var pp in this.Node.Nodes.Values)
                {
                    ...
                    var y = -((this.isOlderVer2 * pp.y) - this._CenterPos.Y) * this.scaleY;
